...
Deno.serve(async (req) => {
  ...
  if (!membership) {
    ...
    return json({ error: "No active reporting membership" }, 403);
  }
  if (report === "settlement_summary" && !["finance", "owner", "admin"].includes(membership.role)) {
    await admin.from("agent_query_audit").insert({ request_id: requestId, user_id: user.id, role: membership.role, report_name: report, filters, outcome: "denied" });
  ...
  if (report === "fleet_status") {
    let q = admin.from("trucks").select("truck_number,dispatcher,insurance,make,odometer_miles,owner,last_known_address,model_year,license_plate,yard_location,samsara_last_connected_at,mechanic_status").limit(500);
    ...
    const { data: rows, error: queryError } = await admin.from("returns").select('Insurance,Truck,"Driver Name","Return Date"').limit(1000);
    error = queryError;
    ...
    let q = admin.from("DriverPay").select('Truck_Number,"Out Date","Return Date","Driver Name",First_Name:"First Name",Last_Name:"Last Name",Solo_Driver_if_1,owner,Dispatch_Name_').limit(500);
    ...
    let q = admin.from("settlements").select('Truck,Dispatch,Owner,Gross,tonu,"Total Expenses",Net,From,To,"Total Driver Pay","Fuel Expenses",Driven_miles,"To Report"').limit(1000);
  ...
  if (error) {
    await admin.from("agent_query_audit").insert({ request_id: requestId, user_id: user.id, role: membership.role, report_name: report, filters, outcome: "invalid" });
    return json({ error: "Report query failed", detail: error.message, request_id: requestId }, 500);
  }
  await admin.from("agent_query_audit").insert({ request_id: requestId, user_id: user.id, role: membership.role, report_name: report, filters, row_count: data.length, outcome: "success" });
  return json({ report, filters, row_count: data.length, as_of: new Date().toISOString(), data, request_id: requestId });
});
