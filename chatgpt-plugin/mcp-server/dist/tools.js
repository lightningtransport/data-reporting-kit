...
export function registerTools(server, client, documents) {
    ...
    }, async ({ query }) => result(searchDocuments(documents, query), `Found documentation for: ${query}.`));
    server.registerTool("fetch", {
        ...
        outputSchema: output,
        annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    }, async ({ id }) => result(fetchDocument(documents, id), `Fetched canonical document ${id}.`));
    ...
    server.registerTool("get_oos_trucks", {
        ...
        outputSchema: output,
        annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    }, async ({ minHoursOOS, limit: requestedLimit }) => {
        if (minHoursOOS > 0) {
            return result({
...
function evidence(response, source, filters) {
    return {
