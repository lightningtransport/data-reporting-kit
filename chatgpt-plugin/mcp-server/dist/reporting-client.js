...
export class ReportingClient {
    ...
    async metadata(report) {
        return this.get({ report, metadata: "true" });
    }
    async query(report, filters, options = {}) {
        for (const [name, value] of Object.entries(filters)) {
            if (!allowedParameters.has(name))
                throw new Error(`Unsupported reporting filter: ${name}`);
            if (value === "")
                throw new Error(`Reporting filter cannot be blank: ${name}`);
        }
    ...
    async get(parameters) {
        ...
        try {
            ...
            if (!response.ok)
                throw new ReportingApiError(response.status, messageForStatus(response.status));
