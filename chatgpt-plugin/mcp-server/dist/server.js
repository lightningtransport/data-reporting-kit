...
export function createMcpServer() {
    const server = new McpServer({ name: "lightning-transport-reporting", version: "0.2.0" }, {
        instructions: "This is a read-only Lightning reporting server. Use live report metadata before unfamiliar queries, never request SQL or mutations, and state source, filters, period, counts, pagination completeness, as_of, freshness limitations, and material caveats.",
    });
...
const httpServer = createHttpServer(async (request, response) => {
    try {
        ...
        if (!auth.ok)
            return sendAuthChallenge(response, auth.message ?? "Authentication required.");
...
async function authenticate(request) {
    if (config.MCP_AUTH_MODE === "development")
        return { ok: true };
...
function sendAuthChallenge(response, message) {
