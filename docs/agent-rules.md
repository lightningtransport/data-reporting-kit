...
- For a return-period `Dispatcher` or `Owner` breakdown, use exact `dispatcher` / `owner` filters on `returns` with inclusive `return_from` / `return_to` and count distinct numeric `Truck`. These columns are truck dispatch/owner on the stored Returns row, not current `trucks` attributes. Do not extend settlement `shared_owner` matching to returns, and do not describe filtered Returns counts as attributed counts for the entire two-source union without reconciling DriverPay separately.
...
| Question type | URL |
|---|---|
| Settlements / settlement dashboard | https://lightning-settlement-dashboard.vercel.app |
...
| Diesel / fuel gallons and spend by month and owner | https://lightning-settlement-dashboard.vercel.app/diesel |

Settlements required sections: slim **Settlements** header, truck focus card under filters, **Summary** from `settlements`, weekly/monthly review, truck and owner rankings (preview + Show all for the full physical-truck selection), fuel spend by owner, KPI strip, closed **Technical details** evidence accordion. Owner/team uses a shadcn Popover + Command multi-select, not a native multi `<select>`. Historical dispatch uses `settlements.Dispatch`. Do not show Full Week or Other Deductions+Previous. Visible copy is English operational wording; kit evidence stays in the accordion. Out Schedule, Trucks Return, and Diesel share the same shell and top-right **Views** menu.
