...
3. By default, the key has full read access to all seven approved reports, including `fuel`, and their explicit sensitive-field allowlists. Set `AGENT_ALLOW_SENSITIVE_<n>=false` only when a particular key needs to be restricted.
